...
import { CollectionBaseSchema, PaginationedSerachParamsSchema } from "../validations";
...
export async function getSavedQuestions(
	...
	const valdiationResult = await action({
		params,
		schema: PaginationedSerachParamsSchema,
		authorize: true,
	});
	if (valdiationResult instanceof Error) {
		return handleError(valdiationResult) as ErrorResponse;
	}
	const { page = 1, pageSize = 10, query, filter } = valdiationResult.params!;
	const userId = valdiationResult.session?.user?.id;
