...
import { Answer, Collection, Interaction, Tag, TagQuestion, Vote } from "@/database";
...
export async function editQuestion(params: EditQustionParams): Promise<ActionResponse<Question>> {
	const validationResult = await action({
		params,
...
export async function deleteQuestion(params: DeleteQuestionParams): Promise<ActionResponse> {
	...
	try {
		...
		//Delete related docuemnts in TagQuestion
		await TagQuestion.deleteMany({ question: questionId }, { session });
		//Keep the tag but decreemnt their question counn
		...
		//Delete all upvotes and downvotes related to the questin
